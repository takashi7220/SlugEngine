...
- `engine/thirdparty/` 以下は外部ライブラリとして扱い、明示的に依頼されない限り直接編集しない。
- 既存コードのスタイル、命名、レイヤー境界を優先する。
- 変更後は、可能な範囲でビルドまたは対象に近いコンパイル確認を行う。
...
| STL 互換 getter | lowercase | `get()`, `reset()`, `swap()` |
