...
{
    render_core::BlendOptions blendOptions;
    fbs::srtf::BlendOptions fbsBlendOptions;
};

static BlendOptionsMapping BLEND_OPTIONS_MAPPING[] =
{
    {render_core::BlendOptions::Opaque, fbs::srtf::BlendOptions::BlendOptions_Opaque},
    {render_core::BlendOptions::Mask, fbs::srtf::BlendOptions::BlendOptions_Mask},
    {render_core::BlendOptions::Transparent, fbs::srtf::BlendOptions::BlendOptions_Transparent},
    {render_core::BlendOptions::Addtive, fbs::srtf::BlendOptions::BlendOptions_Addtive},
    {render_core::BlendOptions::Modulate, fbs::srtf::BlendOptions::BlendOptions_Modulate}
};

render_core::BlendOptions ConverteBlendOptions(const fbs::srtf::BlendOptions options)
{
...
void ParseConstantParameter(render_core::ConstantParameter* dstConstantParameter, const fbs::srtf::ConstantParameter* srcConstantParameter)
{
...
void ParseStaticSwitches(render_core::ShaderParameter* dstShader, const flatbuffers::Vector<::flatbuffers::Offset<slug::fbs::srtf::BoolParameter>>* staticSwitches)
{
...
void ParseShaderItem(render_core::RenderService& service, render_core::ShaderPtr& dstShader, const fbs::srtf::ShaderItem* shaderItem, render_core::ShaderType type, core::StringView shaderName)
{
    render_core::Shader::SetupParam param;
    param.type = type;
...
void ParseShaderItems(render_core::RenderService& service, render_core::ShaderPtr& dstShader, const fbs::srtf::Shader* shader)
{
    for (uint32_t i = 0; i < shader->vertex_shaders()->size(); i++)
    {
        ParseShaderItem(context, dstShader, shader->vertex_shaders()->Get(i), render_core::ShaderType::Vertex, shader->name()->c_str());
    }

    for (uint32_t i = 0; i < shader->pixel_shaders()->size(); i++)
    {
        ParseShaderItem(context, dstShader, shader->pixel_shaders()->Get(i), render_core::ShaderType::Pixel, shader->name()->c_str());
    }

    for (uint32_t i = 0; i < shader->geometry_shaders()->size(); i++)
    {
        ParseShaderItem(context, dstShader, shader->geometry_shaders()->Get(i), render_core::ShaderType::Geometry, shader->name()->c_str());
    }

    for (uint32_t i = 0; i < shader->hull_shaders()->size(); i++)
    {
        ParseShaderItem(context, dstShader, shader->hull_shaders()->Get(i), render_core::ShaderType::Hull, shader->name()->c_str());
    }

    for (uint32_t i = 0; i < shader->domain_shaders()->size(); i++)
    {
        ParseShaderItem(context, dstShader, shader->domain_shaders()->Get(i), render_core::ShaderType::Domain, shader->name()->c_str());
    }

    for (uint32_t i = 0; i < shader->compute_shaders()->size(); i++)
    {
        ParseShaderItem(context, dstShader, shader->compute_shaders()->Get(i), render_core::ShaderType::Compute, shader->name()->c_str());
    }
}

void ParseDrawSettings(render_core::ShaderParameter* dstShader, const fbs::srtf::DrawSettings* drawSettings)
{
    render_core::DrawSettings dstDrawSettings = {};
    dstDrawSettings.blendOptions = ConverteBlendOptions(drawSettings->blend());
    dstDrawSettings.shadingModel = render_core::ShadingModel { drawSettings->shading_model()->name()->c_str(), drawSettings->shading_model()->id() };
    dstDrawSettings.giSettings = render_core::GiSettings { drawSettings->gi_settings()->diffuse_boost(), drawSettings->gi_settings()->emissive_boost() };
...
bool ShaderParser::Load(const InputParam& input, OutParam& out)
{
    ...
    out.shader = core::MakeReference<render_core::Shader>();
...
bool MaterialParser::Load(const InputParam& input, OutParam& out)
{
    ...
    out.material = core::MakeReference<render_core::Material>();
