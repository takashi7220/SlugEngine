...
ShaderAsset::ShaderAsset(engine_core::AssetHeader header, render_core::RenderService& service)
...
bool ShaderAsset::Resolve(const engine_core::AssetResolver& resolver)
{
    for (auto& textureSlot : m_textureSlots)
    {
        ...
        m_shader->AddTexture(render_core::SlotInfo {textureSlot.slotName.c_str()}, textureAsset->GetTexture());
    }
    return true;
}
...
const render_core::ShaderPtr& ShaderAsset::GetShader()
{
    return m_shader;
}
