...
GraphicsAssetLoader::GraphicsAssetLoader(render_core::RenderService& service)
    : m_context(context)
{
...
core::TReferencePtr<GraphicsAssetBase> GraphicsAssetUtility::Generate(const engine_core::AssetHeader& header, render_core::RenderService& service)
{
    engine_core::AssetType type(header.assetType);
    if (GRAPHICS_ASSET_GENERATOR_TABLE.find(type) != GRAPHICS_ASSET_GENERATOR_TABLE.end())
    {
    ...
    SLUG_ASSERT_MSG(false, core::StringUtility::Snprintf("Cant Find Asset Generator [%s]", type.name.c_str()).c_str());
    return nullptr;
}
