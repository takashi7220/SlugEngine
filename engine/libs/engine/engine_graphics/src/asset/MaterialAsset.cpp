...
MaterialAsset::MaterialAsset(engine_core::AssetHeader header, render_core::RenderService& service)
...
bool MaterialAsset::Resolve(const engine_core::AssetResolver& resolver)
{
    ...
    for (auto& slot : m_textureSlots)
    {
        if (resolver.HasAsset(slot.textureId))
        {
            ...
            m_material->AddTexture(render_core::SlotInfo {slot.slotName.c_str()}, slot.texture->GetTexture());
        }
    }
    UpdateMaterial();
    return true;
...
render_core::MaterialPtr MaterialAsset::GetMaterial()
{
    return m_material;
}
