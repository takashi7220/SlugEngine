...
SLUG_NODISCARD core::TReferencePtr<BufferChunk> UploadManager::CreateChunk(size_t size) const
{
    ...
    core::TReferencePtr<BufferChunk> chunk = core::NewObject<BufferChunk>();
...
{
    Texture* t = core::SObjectUtility::CheckedCast<Texture>(itexture);
...
{
    Texture* dst = core::SObjectUtility::CheckedCast<Texture>(idst);
    Texture* src = core::SObjectUtility::CheckedCast<Texture>(isrc);
...
{
    Texture* src = core::SObjectUtility::CheckedCast<Texture>(isrc);
    StagingTexture* dst = core::SObjectUtility::CheckedCast<StagingTexture>(idst);
...
{
    StagingTexture* src = core::SObjectUtility::CheckedCast<StagingTexture>(isrc);
    Texture* dst = core::SObjectUtility::CheckedCast<Texture>(idst);
...
void CommandList::ClearBufferUInt(IBuffer* ibuffer, uint32_t clearValue)
{
    ...
    {
        core::String str = core::StringUtility::Snprintf("Cannot clear buffer %s, because it was created with canHaveUAVs = false", buffer->desc.debugName.c_str());
        return;
    }
...
{
    Buffer* dst = core::SObjectUtility::CheckedCast<Buffer>(idst);
    Buffer* src = core::SObjectUtility::CheckedCast<Buffer>(isrc);
...
void CommandList::SetPushConstants(const void* data, size_t byteSize)
{
    ...
    {
        GraphicsPipeline* pso = core::SObjectUtility::CheckedCast<GraphicsPipeline>(m_currentGraphicsState.pipeline);
        rootsig = pso->rootSignature;
        isGraphics = true;
    }
    else if (m_currentComputeStateValid && m_currentComputeState.pipeline)
    {
        ComputePipeline* pso = core::SObjectUtility::CheckedCast<ComputePipeline>(m_currentComputeState.pipeline);
        rootsig = pso->rootSignature;
        isGraphics = false;
    }
    else if (m_currentRayTracingStateValid && m_currentRayTracingState.shaderTable)
    {
        RayTracingPipeline* pso = core::SObjectUtility::CheckedCast<RayTracingPipeline>(m_currentRayTracingState.shaderTable->GetPipeline());
        rootsig = pso->globalRootSignature;
        isGraphics = false;
    }
    else if (m_currentMeshletStateValid && m_currentMeshletState.pipeline)
    {
        MeshletPipeline* pso = core::SObjectUtility::CheckedCast<MeshletPipeline>(m_currentMeshletState.pipeline);
        rootsig = pso->rootSignature;
        isGraphics = true;
    }
...
{
    GraphicsPipeline* pso = core::SObjectUtility::CheckedCast<GraphicsPipeline>(state.pipeline);
    FrameBuffer* framebuffer = core::SObjectUtility::CheckedCast<FrameBuffer>(state.framebuffer);
    ...
    const bool updateRootSignature = !m_currentGraphicsStateValid || m_currentGraphicsState.pipeline == nullptr ||
        core::SObjectUtility::CheckedCast<GraphicsPipeline>(m_currentGraphicsState.pipeline)->rootSignature != pso->rootSignature;
    ...
    if (updateIndexBuffer)
    {
        ...
        {
            Buffer* buffer = core::SObjectUtility::CheckedCast<Buffer>(state.indexBuffer.buffer);
    ...
    if (updateVertexBuffers)
    {
        ...
        InputLayout* inputLayout = core::SObjectUtility::CheckedCast<InputLayout>(pso->desc.inputLayout.get());

        for (size_t i = 0; i < state.vertexBuffers.size(); i++)
        {
            const VertexBufferBinding& binding = state.vertexBuffers[i];

            Buffer* buffer = core::SObjectUtility::CheckedCast<Buffer>(binding.buffer);
    ...
    if (updateShadingRate || updateFramebuffer)
    {
        ...
        if (shouldEnableVariableRateShading)
        {
            ...
            Texture* texture = core::SObjectUtility::CheckedCast<Texture>(framebufferDesc.shadingRateAttachment.texture);
...
{
    Buffer* indirectParams = core::SObjectUtility::CheckedCast<Buffer>(m_currentGraphicsState.indirectParams);
...
{
    ComputePipeline* pso = core::SObjectUtility::CheckedCast<ComputePipeline>(state.pipeline);

    const bool updateRootSignature = !m_currentComputeStateValid || m_currentComputeState.pipeline == nullptr ||
        core::SObjectUtility::CheckedCast<ComputePipeline>(m_currentComputeState.pipeline)->rootSignature != pso->rootSignature;
...
{
    Buffer* indirectParams = core::SObjectUtility::CheckedCast<Buffer>(m_currentComputeState.indirectParams);
    SLUG_ASSERT(indirectParams);
    UpdateComputeVolatileBuffers();
...
{
    MeshletPipeline* pso = core::SObjectUtility::CheckedCast<MeshletPipeline>(state.pipeline);
    FrameBuffer* framebuffer = core::SObjectUtility::CheckedCast<FrameBuffer>(state.framebuffer);
    ...
    const bool updateRootSignature = !m_currentMeshletStateValid || m_currentMeshletState.pipeline == nullptr ||
        core::SObjectUtility::CheckedCast<MeshletPipeline>(m_currentMeshletState.pipeline)->rootSignature != pso->rootSignature;
...
{
    ShaderTable* shaderTable = core::SObjectUtility::CheckedCast<ShaderTable>(state.shaderTable);
        ...
        auto writeEntry = [this, entrySize, &cpuVA, &gpuVA, &entryIndex](const ShaderTable::Entry& entry)
            {
                ...
                {
                    d3d12::BindingSet* bindingSet = core::SObjectUtility::CheckedCast<d3d12::BindingSet>(entry.localBindings.get());
...
void CommandList::BuildBottomLevelAccelStruct(rt::IAccelStruct* ias, const rt::GeometryDesc* pGeometries, size_t numGeometries, rt::AccelStructBuildFlagsBitset buildFlags)
{
    ...
    for (uint32_t i = 0; i < numGeometries; i++)
    {
        ...
        if (geometryDesc.geometryType == rt::GeometryType::Triangles)
        {
            ...
            OpacityMicromap* om = triangles.opacityMicromap ? core::SObjectUtility::CheckedCast<OpacityMicromap>(triangles.opacityMicromap) : nullptr;
    ...
    if (!core::SObjectUtility::CheckedCast<d3d12::Device>(m_device)->GetAccelStructPreBuildInfo(ASPreBuildInfo, as->GetDesc()))
    {
        return;
    }
    ...
    {
        core::String str = core::StringUtility::Snprintf("BLAS %s build requires at least %ld, bytes", as->desc.debugName.c_str(), ASPreBuildInfo.ResultDataMaxSizeInBytes);
        str += core::StringUtility::Snprintf(" in the data buffer, while the allocated buffer is only %ld bytes", as->dataBuffer->desc.byteSize);
    ...
    {
        core::String str = core::StringUtility::Snprintf("Couldn't suballocate a scratch buffer for BLAS %s The build requires %d bytes of scratch space.", as->desc.debugName.c_str(), scratchSize);
        SLUG_LOG(RHILogCategory, core::LogType::Error, str.c_str());
        return;
    }
...
void CommandList::BuildTopLevelAccelStruct(rt::IAccelStruct* ias, const rt::InstanceDesc* pInstances, size_t numInstances, rt::AccelStructBuildFlagsBitset buildFlags)
{
    ...
    for (uint32_t i = 0; i < numInstances; i++)
    {
        ...
        {
            AccelStruct* blas = core::SObjectUtility::CheckedCast<AccelStruct>(instance.bottomLevelAs);
...
void CommandList::SetResourceStatesForBindingSet(IBindingSet* ibindingSet)
{
    ...
    BindingSet* bindingSet = core::SObjectUtility::CheckedCast<BindingSet>(ibindingSet);
    ...
    for (auto bindingIndex : bindingSet->bindingsThatNeedTransitions)
    {
        ...
        case ResourceType::Texture_SRV:
            RequireTextureState(core::SObjectUtility::CheckedCast<ITexture>(binding.resourcePtr), binding.subresources, ResourceStates::ShaderResource);
            break;

        case ResourceType::Texture_UAV:
            RequireTextureState(core::SObjectUtility::CheckedCast<ITexture>(binding.resourcePtr), binding.subresources, ResourceStates::UnorderedAccess);
            break;

        case ResourceType::TypedBuffer_SRV:
        case ResourceType::StructuredBuffer_SRV:
        case ResourceType::RawBuffer_SRV:
            RequireBufferState(core::SObjectUtility::CheckedCast<IBuffer>(binding.resourcePtr), ResourceStates::ShaderResource);
            break;

        case ResourceType::TypedBuffer_UAV:
        case ResourceType::StructuredBuffer_UAV:
        case ResourceType::RawBuffer_UAV:
            RequireBufferState(core::SObjectUtility::CheckedCast<IBuffer>(binding.resourcePtr), ResourceStates::UnorderedAccess);
            break;

        case ResourceType::ConstantBuffer:
            RequireBufferState(core::SObjectUtility::CheckedCast<IBuffer>(binding.resourcePtr), ResourceStates::ConstantBuffer);
            break;

        case ResourceType::RayTracingAccelStruct:
            RequireBufferState(core::SObjectUtility::CheckedCast<AccelStruct>(binding.resourcePtr)->dataBuffer, ResourceStates::AccelStructRead);
            break;
    ...
    if (bindingUpdateMask)
    {
        ...
        for (uint32_t bindingSetIndex = 0; bindingSetIndex < uint32_t(bindings.size()); bindingSetIndex++)
        {
            ...
            if (_bindingSet->GetDesc())
            {
                ...
                BindingSet* bindingSet = core::SObjectUtility::CheckedCast<BindingSet>(_bindingSet);
                ...
                for (size_t volatileCbIndex = 0; volatileCbIndex < bindingSet->rootParametersVolatileCb.size(); volatileCbIndex++)
                {
                    ...
                    if (parameter.second)
                    {
                        Buffer* buffer = core::SObjectUtility::CheckedCast<Buffer>(parameter.second);
            ...
            {
                DescriptorTable* descriptorTable = core::SObjectUtility::CheckedCast<DescriptorTable>(_bindingSet);
                ...
                for (size_t volatileCbIndex = 0; volatileCbIndex < bindingSet->rootParametersVolatileCb.size(); volatileCbIndex++)
                {
                    ...
                    if (parameter.second)
                    {
                        ...
                        if (buffer->desc.volatileBuffer)
                        {
                            ...
                            if (!volatileData)
                            {
                                core::String str = core::StringUtility::Snprintf("Attempted use of a volatile constant buffer %s before it was written into", buffer->desc.debugName.c_str());
                                SLUG_LOG(RHILogCategory, core::LogType::Error, str.c_str());
                                continue;
                            }
...
void CommandList::BuildTopLevelAccelStructInternal(AccelStruct* as, D3D12_GPU_VIRTUAL_ADDRESS instanceData, size_t numInstances, rt::AccelStructBuildFlagsBitset buildFlags)
{
    ...
    if (ASPreBuildInfo.ResultDataMaxSizeInBytes > as->dataBuffer->desc.byteSize)
    {
        core::String str = core::StringUtility::Snprintf("TLAS %s build requires at least %d bytes in the data buffer, while the allocated buffer is only %d bytes",
    ...
    {
        core::String str = core::StringUtility::Snprintf("Couldn't suballocate a scratch buffer for TLAS %s The build requires %d bytes of scratch space.",
