...
SwapChainD3D12* SwapChainD3D12::Create(const IContext::WindowParam& param, core::TReferencePtr<ID3D12CommandQueue>& graphicsQueue, bool enableDebug)
{
    ...
    SwapChainD3D12* swapChain = core::NewObject<SwapChainD3D12>();
    ...
    if (FAILED(hr))
    {
        core::String str = core::StringUtility::Snprintf("Error : CreateDXGIFactory2 hr = %x", hr);
...
bool ContextD3D12::Create(const SetupParam& param)
{
    ...
    {
        core::TReferencePtr<ID3D12Debug> debug = nullptr;
        ...
        core::TReferencePtr<ID3D12Debug1> debug1 = nullptr;
        if (debug && SUCCEEDED(debug->QueryInterface(IID_PPV_ARGS(&debug1))))
        {
    ...
    if (FAILED(hr))
    {
        SLUG_D3D12_HRESULT_ASSERT(hr, m_device)
        Destroy();
        return false;
    }

    if (m_param.enableDebug && m_device)
    {
        core::TReferencePtr<ID3D12InfoQueue> infoQueue = nullptr;
        ...
        if (SUCCEEDED(hr) && infoQueue)
        {
            ...
            filter.DenyList.NumIDs = static_cast<UINT>(std::size(disableMessageIDs));
...
{
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    core::TReferencePtr<IContext> device = core::NewObject<ContextD3D12>();
    device->Create(param);
    return device;
}
