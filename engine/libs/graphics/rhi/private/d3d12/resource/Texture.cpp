...
void Texture::CreateUAV(size_t descriptor, FormatType format, TextureDimension dimension, TextureSubresourceSet subresources) const
{
    ...
    case TextureDimension::Texture2DMSArray:
        {
            core::String txt = {};
            core::StringUtility::Snprintf(txt, "Error : Texture [%s] has unsupported dimension for UAV, %s", desc.debugName.c_str(), Utility::TextureDimensionToString(desc.dimension));
...
void Texture::CreateDSV(size_t descriptor, TextureSubresourceSet subresources, bool isReadOnly) const
{
    ...
    case TextureDimension::Texture3D:
        {
            core::String txt = {};
            core::StringUtility::Snprintf(txt, "Error : Texture [%s] has unsupported dimension for DSV, %s", desc.debugName.c_str(), Utility::TextureDimensionToString(desc.dimension));
            SLUG_ASSERT_MSG(false, txt.c_str());
            return;
        }
    case TextureDimension::Unknown:
