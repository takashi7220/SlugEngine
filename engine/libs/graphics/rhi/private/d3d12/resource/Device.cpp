...
{
    core::TReferencePtr<Device> device = core::MakeReference<Device>(desc);
    return device;
}
...
HeapPtr Device::CreateHeap(const HeapDesc& d)
{
    ...
    case HeapType::ReadBack:
        ...
        break;
    default:
        core::String str = core::StringUtility::Snprintf("InvalidEnum HeapType %d", d.type);
    ...
    if (FAILED(res))
    {
        core::String str = core::StringUtility::Snprintf("CreateHeap call failed for heap %s, HRESULT = %x", d.debugName.c_str(), res);
        SLUG_LOG(RHILogCategory, core::LogType::Alert, str.c_str());
        return nullptr;
    }
    ...
    core::TReferencePtr<Heap> heap = core::MakeReference<Heap>();
    heap->heap = d3dHeap;
    heap->desc = d;
    return heap;
}
...
TexturePtr Device::CreateTexture(const TextureDesc& d)
{
    ...
    core::TReferencePtr<Texture> texture = core::MakeReference<Texture>(m_context, m_resources, d, rd);
    ...
    if (FAILED(hr))
    {
        core::String str = core::StringUtility::Snprintf("Failed to create texture %s, error code = %x", d.debugName.c_str(), hr);
        SLUG_LOG(RHILogCategory, core::LogType::Alert, str.c_str());
    ...
    if (isShared)
    {
        ...
        if (FAILED(hr))
        {
            core::String str = core::StringUtility::Snprintf("Failed to create shared handle %s, error code = %x", d.debugName.c_str(), hr);
            SLUG_LOG(RHILogCategory, core::LogType::Alert, str.c_str());
...
{
    Texture* tmpTexture = core::SObjectUtility::CheckedCast<Texture>(texture);
...
{
    Texture* texture = core::SObjectUtility::CheckedCast<Texture>(itexture);
    Heap* heap = core::SObjectUtility::CheckedCast<Heap>(iheap);
    ...
    if (FAILED(hr))
    {
        core::String str = core::StringUtility::Snprintf("Failed to create placed texture %s, errod code = %x", texture->desc.debugName.c_str(), hr);
        SLUG_LOG(RHILogCategory, core::LogType::Alert, str.c_str());
        return false;
    }
...
TexturePtr Device::CreateHandleForNativeTexture(ObjectType objectType, Object itexture, const TextureDesc& desc)
{
    ...
    core::TReferencePtr<Texture> texture = core::MakeReference<Texture>(m_context, m_resources, desc, pResource->GetDesc());
...
StagingTexturePtr Device::CreateStagingTexture(const TextureDesc& d, CpuAccessMode cpuAccess)
{
    ...
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS_TEXTURE
    StagingTexture* ret = core::NewObject<StagingTexture>();
    ret->desc = d;
    ...
    ret->buffer = core::SObjectUtility::CheckedCast<Buffer>(buffer.get());
    if (!ret->buffer)
    {
        delete ret;
        return nullptr;
...
{
    StagingTexture* tex = core::SObjectUtility::CheckedCast<StagingTexture>(itex);
    ...
    if (FAILED(hr))
    {
        core::String str = core::StringUtility::Snprintf("Map call failed for textre %s, error code %x", tex->desc.debugName.c_str(), hr);
        SLUG_LOG(RHILogCategory, core::LogType::Alert, str.c_str());
        return nullptr;
    }
...
{
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS_SAMPLER
    Sampler* sampler = core::NewObject<Sampler>(m_context, d);
...
BufferPtr Device::CreateBuffer(const BufferDesc& d)
{
    ...
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS_BUFFER
    Buffer* buffer = core::NewObject<Buffer>(m_context, m_resources, desc);
    ...
    if (FAILED(hr))
    {
        core::String str = core::StringUtility::Snprintf("CreateCommittedResource call failed for buffer %s, error code %x", d.debugName.c_str(), hr);
        SLUG_LOG(RHILogCategory, core::LogType::Alert, str.c_str());
...
{
    Buffer* b = core::SObjectUtility::CheckedCast<Buffer>(ib);
    ...
    if (FAILED(hr))
    {
        core::String str = core::StringUtility::Snprintf("Map call failed for buffer %s, error code %x", b->desc.debugName.c_str(), hr);
        SLUG_LOG(RHILogCategory, core::LogType::Alert, str.c_str());
        return nullptr;
    }
...
{
    Buffer* buffer = core::SObjectUtility::CheckedCast<Buffer>(ibuffer);
...
bool Device::BindBufferMemory(IBuffer* ibuffer, IHeap* iheap, uint64_t offset)
{
    ...
    if (FAILED(hr))
    {
        core::String str = core::StringUtility::Snprintf("Failed to create placed buffer %s, error code %x", buffer->desc.debugName.c_str(), hr);
        SLUG_LOG(RHILogCategory, core::LogType::Alert, str.c_str());
        return false;
    }
...
ShaderPtr Device::CreateShader(const ShaderDesc& desc, const void* binary, size_t binarySize)
{
    ...
    // シェーダのバイナリデータをコピー
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS_SHADER
    Shader* shader = core::NewObject<Shader>();
...
{
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS_SHADER
    ShaderLibrary* shaderLibrary = core::NewObject<ShaderLibrary>();
...
InputLayoutPtr Device::CreateInputLayout(const VertexAttributeDesc* desc, uint32_t attributeCount, IShader* vertexShader)
{
    ...
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    InputLayout* layout = core::NewObject<InputLayout>();
...
{
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    EventQuery* ret = core::NewObject<EventQuery>();
    return EventQueryPtr::create(ret);
}

void Device::SetEventQuery(IEventQuery* iquery, CommandQueue queue)
{
    EventQuery* query = core::SObjectUtility::CheckedCast<EventQuery>(iquery);
    Queue* pQueue = GetQueue(queue);
...
TimerQueryPtr Device::CreateTimerQuery()
{
    if (!m_context.timerQueryHeap)
    {
        ...
        if (!m_context.timerQueryHeap)
        {
            ...
            m_context.timerQueryResolveBuffer = core::SObjectUtility::CheckedCast<Buffer>(timerQueryBuffer.get());
        }
    }
    ...
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    TimerQuery* query = core::NewObject<TimerQuery>(m_resources);
...
{
    TimerQuery* query = core::SObjectUtility::CheckedCast<TimerQuery>(iquery);
...
{
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    FrameBuffer* fb = core::NewObject<FrameBuffer>(m_resources);
    fb->desc = desc;
    fb->frameBufferInfo = FrameBufferInfoEx(desc);

    if (!desc.colorAttachments.empty())
    {
        Texture* texture = core::SObjectUtility::CheckedCast<Texture>(desc.colorAttachments[0].texture);
        fb->rtWidth = texture->desc.width;
        fb->rtHeight = texture->desc.height;
    }
    else if (desc.depthAttachment.Valid())
    {
        Texture* texture = core::SObjectUtility::CheckedCast<Texture>(desc.depthAttachment.texture);
        fb->rtWidth = texture->desc.width;
        fb->rtHeight = texture->desc.height;
    }
    ...
    for (size_t rt = 0; rt < desc.colorAttachments.size(); rt++)
    {
        ...
        Texture* texture = core::SObjectUtility::CheckedCast<Texture>(attachment.texture);
...
core::TReferencePtr<RootSignature> Device::GetRootSignature(const core::TStaticVector<BindingLayoutPtr, MAX_BINDING_LAYOUTS>& pipelineLayouts, bool allowInputLayout, core::TReferencePtr<RootSignature> pRS)
{
    ...
    if (!rootsig)
    {
        rootsig = core::SObjectUtility::CheckedCast<RootSignature>(BuildRootSignature(pipelineLayouts, allowInputLayout, false).get());
        rootsig->hash = hash;
...
GraphicsPipelinePtr Device::CreateHandleForNativeGraphicsPipeline(IRootSignature* rootSignature, ID3D12PipelineState* pipelineState, const GraphicsPipelineDesc& desc, const FrameBufferInfo& framebufferInfo)
{
    ...
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    GraphicsPipeline* pso = core::NewObject<GraphicsPipeline>();
    pso->desc = desc;
    pso->framebufferInfo = framebufferInfo;
    pso->rootSignature = core::SObjectUtility::CheckedCast<RootSignature>(rootSignature);
...
core::TReferencePtr<ID3D12PipelineState> Device::CreatePipelineState(const GraphicsPipelineDesc& desc, RootSignature* pRS, const FrameBufferInfo& fbinfo) const
{
    ...
    Shader* shader;
    shader = core::SObjectUtility::CheckedCast<Shader>(desc.vs.get());
    if (shader)
    {
        d3d12Desc.VS = { &shader->bytecode[0], shader->bytecode.size() };
    }

    shader = core::SObjectUtility::CheckedCast<Shader>(desc.hs.get());
    if (shader)
    {
        d3d12Desc.HS = { &shader->bytecode[0], shader->bytecode.size() };
    }

    shader = core::SObjectUtility::CheckedCast<Shader>(desc.ds.get());
    if (shader)
    {
        d3d12Desc.DS = { &shader->bytecode[0], shader->bytecode.size() };
    }

    shader = core::SObjectUtility::CheckedCast<Shader>(desc.gs.get());
    if (shader)
    {
        d3d12Desc.GS = { &shader->bytecode[0], shader->bytecode.size() };
    }

    shader = core::SObjectUtility::CheckedCast<Shader>(desc.ps.get());
    if (shader)
    {
    ...
    InputLayout* inputLayout = core::SObjectUtility::CheckedCast<InputLayout>(desc.inputLayout.get());
...
void Device::RegisterShaderCode(ShaderPtr& srcShader, const void* dstShaderBytecodePointer, size_t dstBytecodeLength) const
{
    ...
    core::MemoryUtility::Memcpy(&shader->bytecode[0], shader->bytecode.size(), dstShaderBytecodePointer, dstBytecodeLength);
    srcShader = shader;
}
...
core::TReferencePtr<ID3D12PipelineState> Device::CreatePipelineStateFromCache(const PipelineCache& pipelineCache, GraphicsPipelineDesc& outDesc, core::TReferencePtr<RootSignature>& outRootSignature, FrameBufferInfoEx& outFrameBufferInfo)
{
    ...
    {
        SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
        outRootSignature.attach(core::NewObject<RootSignature>(m_resources));
    }
    ...
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    outDesc.inputLayout = core::NewObject<InputLayout>();
    InputLayout* inputLayout = core::SObjectUtility::CheckedCast<InputLayout>(outDesc.inputLayout.get());
...
ComputePipelinePtr Device::CreateComputePipeline(const ComputePipelineDesc& desc)
{
    ...
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    ComputePipeline* cso = core::NewObject<ComputePipeline>();
...
core::TReferencePtr<ID3D12PipelineState> Device::CreatePipelineState(const ComputePipelineDesc& desc, RootSignature* pRS) const
{
    ...
    d3d12Desc.pRootSignature = pRS->handle;
    Shader* shader = core::SObjectUtility::CheckedCast<Shader>(desc.cs.get());
...
MeshletPipelinePtr Device::CreateHandleForNativeMeshletPipeline(IRootSignature* rootSignature, ID3D12PipelineState* pipelineState, const MeshletPipelineDesc& desc, const FrameBufferInfo& framebufferInfo)
{
    ...
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    MeshletPipeline* pso = core::NewObject<MeshletPipeline>();
    pso->desc = desc;
    pso->framebufferInfo = framebufferInfo;
    pso->rootSignature = core::SObjectUtility::CheckedCast<RootSignature>(rootSignature);
    pso->pipelineState = pipelineState;
...
{
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    RayTracingPipeline* pso = core::NewObject<RayTracingPipeline>(m_context);
    {
...
{
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    BindingLayout* ret = core::NewObject<BindingLayout>(desc);
...
{
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    BindlessLayout* ret = core::NewObject<BindlessLayout>(desc);
    return BindingLayoutPtr::create(ret);
}
...
{
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    BindingSet* ret = core::NewObject<BindingSet>(m_context, m_resources);
    ret->desc = desc;

    BindingLayout* pipelineLayout = core::SObjectUtility::CheckedCast<BindingLayout>(layout);
...
DescriptorTablePtr Device::CreateDescriptorTable(IBindingLayout* layout)
{
    ...
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    DescriptorTable* ret = core::NewObject<DescriptorTable>(m_resources);
    ret->capacity = 0;
...
{
    DescriptorTable* descriptorTable = core::SObjectUtility::CheckedCast<DescriptorTable>(idescriptorTable);
...
bool Device::WriteDescriptorTable(IDescriptorTable* idescriptorTable, const BindingSetItem& binding)
{
    ...
    case ResourceType::Texture_SRV:
        {
            Texture* texture = core::SObjectUtility::CheckedCast<Texture>(binding.resourcePtr);
    ...
    case ResourceType::RawBuffer_SRV:
        {
            Buffer* buffer = core::SObjectUtility::CheckedCast<Buffer>(binding.resourcePtr);
    ...
    case ResourceType::RayTracingAccelStruct:
        {
            AccelStruct* accelStruct = core::SObjectUtility::CheckedCast<AccelStruct>(binding.resourcePtr);
...
RootSignaturePtr Device::BuildRootSignature(const core::TStaticVector<BindingLayoutPtr, MAX_BINDING_LAYOUTS>& pipelineLayouts, bool allowInputLayout, bool isLocal, const D3D12_ROOT_PARAMETER1* pCustomParameters, uint32_t numCustomParameters)
{
    ...
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    RootSignature* rootsig = core::NewObject<RootSignature>(m_resources);
    ...
    for (uint32_t layoutIndex = 0; layoutIndex < uint32_t(pipelineLayouts.size()); layoutIndex++)
    {
        ...
        {
            BindingLayout* layout = core::SObjectUtility::CheckedCast<BindingLayout>(pipelineLayouts[layoutIndex].get());
        ...
        {
            BindlessLayout* layout = core::SObjectUtility::CheckedCast<BindlessLayout>(pipelineLayouts[layoutIndex].get());
            RootParameterIndex rootParameterOffset = RootParameterIndex(rootParameters.size());
    ...
    if (FAILED(res))
    {
        core::String str = core::StringUtility::Snprintf("D3D12SerializeVersionedRootSignature call failed, HRESULT %x, ", res);
        if (errorBlob)
        {
    ...
    if (FAILED(res))
    {
        core::String str = core::StringUtility::Snprintf("CreateRootSignature call failed, HRESULT = %x", res);
        return nullptr;
    }
...
rt::AccelStructPtr Device::CreateAccelStruct(const rt::AccelStructDesc& desc)
{
    ...
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    AccelStruct* as = core::NewObject<AccelStruct>(m_context);
    as->desc = desc;
    ...
    if (need_buffer)
    {
        ...
        BufferPtr buffer = CreateBuffer(bufferDesc);
        as->dataBuffer = core::SObjectUtility::CheckedCast<Buffer>(buffer.get());
    }
...
{
    AccelStruct* as = core::SObjectUtility::CheckedCast<AccelStruct>(ias);
...
rhi::CommandListPtr Device::CreateCommandList(const CommandListParameters& params)
{
    ...
    SLUG_MEMORY_LABEL_SCOPE_GRAPHICS
    return CommandListPtr::create(core::NewObject<CommandList>(this, m_context, m_resources, params));
}
...
uint64_t Device::ExecuteCommandLists(rhi::ICommandList* const* pCommandLists, size_t numCommandLists, CommandQueue executionQueue)
{
    ...
    {
        m_commandListsToExecute[i] = core::SObjectUtility::CheckedCast<CommandList>(pCommandLists[i])->GetD3D12CommandList();
    }
    ...
    for (size_t i = 0; i < numCommandLists; i++)
    {
        auto instance = core::SObjectUtility::CheckedCast<CommandList>(pCommandLists[i])->Executed(pQueue);
