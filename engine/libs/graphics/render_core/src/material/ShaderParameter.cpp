...
void ShaderParameter::Reflesh(RenderService& service)
{
    UpdateBindingResource(service, *this);
}
...
void ShaderParameter::UpdateBindingResource(RenderService& service, const ConstantParameter& constantParameter)
{
    ...
    {
        service.GetResourcePool().GetBufferPool()->Delete(key);
        m_constantBuffer = nullptr;
    }
    ...
    if (m_constantBuffer == nullptr)
    {
        ...
        m_constantBuffer = service.GetDevice()->CreateBuffer(bufferDesc);
        service.GetResourcePool().GetBufferPool()->Add(key, m_constantBuffer);
    }

    if (m_needUpdateBuffer)
    {
        service.GetCommandList(RenderService::CommandListType::Resource)->WriteBuffer(m_constantBuffer, constantParameter.GetConstantBufferPtr().data(), constantParameter.GetCostantBufferSize());
    }
    ...
    m_bindingLayout = binding->GetBindingLayout(service);
    m_bindingSet = binding->GetBindingSet(service, this);
