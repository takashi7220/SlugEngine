...
void SurfacePass::Draw(RenderService& service, const PipelineInfo& pipelineInfo)
{
    ...
    graphicsState.framebuffer = pipelineInfo.framebuffer->GetFramebuffer(service, pipelineInfo.view);
    ...
    while (const DrawObjectPtr item = pipelineInfo.surfaceSettings.drawObjectCuller->GetNextDrawObject())
    {
        ...
        if (newMaterial)
        {
            SetupGraphicsPipeline(service, pipelineInfo.drawSettings,  *pipelineInfo.view, *item, item->GetMaterial(), graphicsState);
...
void SurfacePass::GetBinding(RenderService& service, const PipelineInfo& pipelineInfo, BindingCachePtr& bindingCache)
{
        ...
        rhi::GraphicsState& state)
{
    ...
    auto pipeline = GraphicsPipeline::Create(service, material, pipelineDrawSettings, drawObject.GetSettings(), state, bindingLayouts);
