...
bool ConstantParameter::SetParameterInternal(size_t size, size_t offset, const void* value)
{
    if (value)
    {
        core::MemoryUtility::Memcpy(m_constantBuffer.data() + offset, size, value, size);
        return true;
    }
    return false;
}
...
bool ConstantParameter::GetParameterInternal(size_t size, size_t offset, void* value) const
{
    if (value)
    {
        core::MemoryUtility::Memcpy(value, size, m_constantBuffer.data() + offset, size);
        return true;
    }
    return false;
}
