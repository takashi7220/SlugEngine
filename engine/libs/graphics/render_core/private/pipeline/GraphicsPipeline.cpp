...
GraphicsPipelinePtr GraphicsPipeline::Create(RenderService& service, PipelineCache& cacheData)
{
    GraphicsPipelinePtr pipeline = nullptr;
    GraphicsPipelinePoolPtr& resourcePool = service.GetResourcePool().GetGraphicsPipelinePool();
    ...
    pipeline = core::NewObject<GraphicsPipeline>();
    pipeline->m_pipeline = service.GetDevice()->CreateGraphicsPipelineFromCache(pipelineCache, desc);
    ...
    const core::TVector<rhi::IBindingLayout*> bindingLayouts)
{
    ...
    SetupVertexAttribute(service, state, desc);
    ...
    pipeline->m_pipeline = service.GetDevice()->CreateGraphicsPipeline(desc, state.framebuffer);
...
{
    material->GetShader()->GetVertexShader(desc.vs);
    material->GetShader()->GetPixelShader(desc.ps);
...
void GraphicsPipeline::SetupVertexAttribute(RenderService& service, const rhi::GraphicsState& state, rhi::GraphicsPipelineDesc& desc)
{
    ...
    desc.inputLayout = service.GetDevice()->CreateInputLayout(vertexAttributeDesc.data(), static_cast<uint32_t>(vertexAttributeDesc.size()), desc.vs);
}
