...
ViewPtr View::Create(RenderService& service, core::StringView name)
{
    ViewPtr view = core::NewObject<View>();
    view->SetName(name);
    ...
    BindingCachePtr binding = BindingCache::Create(service, param);
    ...
    {
        view->m_shaderParameter->AddParameter(viewParam.name.c_str(), (render_core::ParameterType)viewParam.type, 1, viewParam.size, nullptr);
    }
    view->m_shaderParameter->Reflesh(service);

    view->m_bindingLayout = binding->GetBindingLayout(service);
    view->m_bindingSet = binding->GetBindingSet(service, view->m_shaderParameter);
    return view;
}
...
void View::Reflesh(RenderService& service)
{
    ...
    m_shaderParameter->Reflesh(service);
}
...
float View::CalculateDrawPixelRate(const BoundingBox& boundingBox) const
{
    math::Vector3 center = boundingBox.Center();
    float radius = boundingBox.Length() * 0.5f;
