...
{
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(Window, IWindow)

    static WindowPtr Create(const Window::Desc& desc);
    explicit Window();
    ~Window();
    ...
    void SetEventFilter(core::TFunctionObject<bool(WindowID, WindowEventType)> filter);
...
private:
    void CreateWindowInternal(const Window::Desc& desc);
    ...
    bool ShouldSendEvent(WindowEventType type) const;
...
private:
    ...
    bool m_autoDestroyOnClose = true;
    ...
    core::TFunctionObject<bool(WindowID, WindowEventType)> m_eventFilter = nullptr;
