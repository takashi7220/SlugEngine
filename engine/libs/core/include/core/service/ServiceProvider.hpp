...
class IService : public core::SObject
{
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(IService, core::SObject)
...
public:
    ...
    {
        return SObjectUtility::CheckedCast<ServiceType, IService>(GetService(ServiceType::GetStaticServiceTypeID()).get());
    }

    template<typename ServiceType>
    TReferencePtr<ServiceType> GetRequiredService()
    {
        return SObjectUtility::CheckedCast<ServiceType, IService>(GetRequiredService(ServiceType::GetStaticServiceTypeID()).get());
    }
private:
...
{
    IServicePtr service = NewObject<ServiceType>(provider.GetRequiredService<Dependends>()...);
