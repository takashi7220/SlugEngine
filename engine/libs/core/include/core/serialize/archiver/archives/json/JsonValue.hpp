...
struct JsonValue
{
    ...
    core::String ToString(int indentSpaces = 4, int depth = 0) const
    {
        ...
        if (IsInteger())
        {
            return core::StringUtility::Snprintf("%lld", static_cast<long long>(AsInteger()));
        }

        if (IsFloating())
        {
            return core::StringUtility::Snprintf("%.17g", AsFloating());
        }
        ...
        if (IsArray())
        {
            ...
            for (const auto& e : AsArray())
            {
                ...
                {
                    out += "\n";
                    out.append(static_cast<size_t>(indentSpaces * (depth + 1)), ' ');
                }
                out += e.ToString(indentSpaces, depth + 1);
            }
            if (pretty)
            {
                out += "\n";
                out.append(static_cast<size_t>(indentSpaces * depth), ' ');
            }
            out += "]";
            return out;
        }
        ...
        for (const auto& [k, v] : AsObject())
        {
            ...
            out += "\"" + EscapeString(k) + "\":";
            if (pretty) out += " ";
            out += v.ToString(indentSpaces, depth + 1);
        }
        if (pretty)
        {
            out += "\n";
