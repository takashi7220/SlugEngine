...
struct ControlBlock : public MemoryObject
{
    ...
    {
        weak.fetch_add(1, core::MemoryOrderRelaxed);
    }

    unsigned long ReleaseWeak() noexcept
    {
        if (weak.fetch_sub(1, core::MemoryOrderAcqRel) == 1)
        {
            delete this;
            return 0;
        }
        return weak.load(core::MemoryOrderRelaxed);
    }

    bool TryIncStrongFromWeak() noexcept
    {
        if (!alive.load(core::MemoryOrderAcquire))
        {
            return false;
        }

        unsigned long s = strong.load(core::MemoryOrderAcquire);
        while (s != 0)
        {
            if (strong.compare_exchange_weak(
                s, s + 1,
                core::MemoryOrderAcqRel,
    ...
    {
        return object_ptr.load(core::MemoryOrderAcquire);
    }

    void ClearObjectPtr() noexcept
    {
        object_ptr.store(nullptr, core::MemoryOrderRelease);
    }
};
...
public:
    ...
    {
        return m_control->strong.fetch_add(1, core::MemoryOrderRelaxed) + 1;
    }

    virtual unsigned long AddWeakRef() noexcept
    {
        m_control->AddWeak();
        return m_control->weak.load(core::MemoryOrderRelaxed);
    }

    virtual unsigned long Release() noexcept
    {
        if (m_control->strong.fetch_sub(1, core::MemoryOrderAcqRel) == 1)
        {
...
protected:
    ReferenceObject()
        : m_control(NewObject<ControlBlock>(this))
    {
        ControlBlock* cb = m_control;
        if (cb)
        {
            cb->strong.store(0, core::MemoryOrderRelaxed);
        }
    }
...
{
    return TReferencePtr<T>(slug::core::NewObject<T>(std::forward<Args>(args)...));
}
