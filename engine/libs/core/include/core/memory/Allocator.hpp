...
public:
    ...
    T* allocate(size_t size)
    {
        ...
        return static_cast<T*>(MemoryUtility::Allocate(size * sizeof(T), static_cast<size_t>(std::align_val_t(alignof(T)))));
    }
    ...
    T* allocate(size_t size, size_t align)
    {
        ...
        return static_cast<T*>(MemoryUtility::Allocate(size * sizeof(T), align));
    }

    void deallocate(T* p, size_t size) noexcept
    {
        MemoryUtility::Deallocate((void*)p);
    }

    void deallocate(T* p, size_t align, size_t size) noexcept
    {
        MemoryUtility::Deallocate((void*)p, align);
    }
