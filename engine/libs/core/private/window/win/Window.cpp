...
WindowPtr Window::Create(const IWindow::Desc& desc)
{
    Window* window = NewObject<Window>();
    ...
    window->m_autoDestroyOnClose = desc.autoDestroyOnClose;
...
String Window::GetTitle() const
{
    ...
    wchar_t title[256] = {};
    GetWindowTextW(m_hwnd, title, len + 1);
    return StringUtility::GetString(title);
}
...
void Window::SetEventFilter(core::TFunctionObject<bool(WindowID, WindowEventType)> filter)
{
...
{
    WinWindowHandle* winHandle = NewObject<WinWindowHandle>();
...
void Window::HandleMessage(UINT msg, WPARAM wp, LPARAM lp, LRESULT& outResult, bool& handled)
{
    ...
    case WM_CLOSE:
        {
            if (ShouldSendEvent(WindowEventType::WindowClose))
            {
                WindowCloseEvent event(GetWindowID());
                m_eventCallback(&event);
            }

            if (m_autoDestroyOnClose)
            {
                this->Destroy();
            }
            handled = m_autoDestroyOnClose;
            outResult = 0;
            return;
        }
    ...
    case WM_SHOWWINDOW:
        {
            ...
            if (ShouldSendEvent(WindowEventType::WindowShow))
            {
                WindowShowEvent event(GetWindowID(), m_shown);
                m_eventCallback(&event);
            }
            handled = false;
            return;
    ...
    case WM_SETFOCUS:
        {
            if (ShouldSendEvent(WindowEventType::WindowFocus))
            {
                WindowFocusEvent event(GetWindowID(), true);
                m_eventCallback(&event);
            }
            handled = false;
            return;
    ...
    case WM_KILLFOCUS:
        {
            if (ShouldSendEvent(WindowEventType::WindowFocus))
            {
                WindowFocusEvent event(GetWindowID(), false);
                m_eventCallback(&event);
            }
            handled = false;
            return;
    ...
    case WM_MOVE:
        {
            if (ShouldSendEvent(WindowEventType::WindowMove))
            {
                RECT rc {};
                GetWindowRect(m_hwnd, &rc);
                WindowMoveEvent event(GetWindowID(), rc.left, rc.top);
                m_eventCallback(&event);
            }
            handled = false;
            return;
    ...
    case WM_SIZE:
        {
            if (wp == SIZE_MINIMIZED)
            {
                ...
                if (ShouldSendEvent(WindowEventType::WindowMinimize))
                {
                    WindowMinimizeEvent event(GetWindowID(), true);
                    m_eventCallback(&event);
                }
            }
            else
            {
                m_allowPresent = true;
                if (ShouldSendEvent(WindowEventType::WindowResize))
                {
                    RECT rc {};
                    GetClientRect(m_hwnd, &rc);
                    float dpi = QueryDpiScale();
                    WindowResizeEvent event(GetWindowID(), rc.right - rc.left, rc.bottom - rc.top, dpi);
                    m_eventCallback(&event);
                }
    ...
    case WM_DPICHANGED:
        {
            ...
            if (ShouldSendEvent(WindowEventType::WindowDpiChanged))
            {
                float dpi = QueryDpiScale();
                WindowDpiChangedEvent event(GetWindowID(), dpi);
                m_eventCallback(&event);
            }
    ...
    case WM_DROPFILES:
        {
            if (m_fileDropEnabled)
            {
                ...
                if (m_eventCallback)
                {
                    WindowFileDropEvent event(GetWindowID(), paths);
                    m_eventCallback(&event);
                }
    ...
    case WM_DESTROY:
        {
            if (ShouldSendEvent(WindowEventType::WindowDestroy))
            {
                WindowDestroyEvent event(GetWindowID());
                m_eventCallback(&event);
            }
            handled = false;
            return;
...
void Window::CreateWindowInternal(const IWindow::Desc& desc)
{
...
bool Window::ShouldSendEvent(WindowEventType type) const
{
    if (!m_eventCallback)
    {
        return false;
    ...
    return m_eventFilter(m_uuid, type);
}
