...
{
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(TestServiceA, IService)
...
{
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(TestServiceB, IService)
...
{
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(TestServiceC, IService)
...
{
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(TestServiceD, IService)
...
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(TestServiceE, IService)
...
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(TestServiceF, IService)
...
{
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(TestServiceBase, IService)
...
{
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(TestServiceG, TestServiceBase)
    SLUG_INTERFACE_SERVICE_REGISTER(TestServiceG)
...
{
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(TestServiceH, IService)
...
{
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(TestServiceI, IService)
    SLUG_SERVICE_REGISTER_DEPENDENTS(TestServiceI, TestServiceH)
    TestServiceI(TestServiceH* serviceH)
    {}
};

class TestServiceJ : public IService
{
public:
    SLUG_DECLARE_SOBJECT_TYPEINFO(TestServiceJ, IService)
    SLUG_SERVICE_CUSTOM_REGISTER(TestServiceJ)
...
TEST(ServiceProvider, ServiceProviderCustomRegister)
{
    ServiceProvider provider;

    TestServiceJ::Desc desc = {};
    desc.value = 10;
    provider.AddSingleton<TestServiceJ>([&desc](slug::core::ServiceProvider provider){
        return NewObject<TestServiceJ>(desc);
    });

    auto serviceJ = provider.GetService<TestServiceJ>();
    EXPECT_EQ(serviceJ->GetValue(), 10);
}
